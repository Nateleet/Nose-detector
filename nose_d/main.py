...
def findLipsOnPhoto(filename):
    try:
        # Подключить каскад для поиска губ
        mouth_cascade = connectCascade('haarcascade_mcs_nose.xml')
...
# Создание окна
window = tk.Tk()
window.title('Nose Detector')              # Задать заголовок окна
window.geometry('400x230')                  # Задать размер окна
window.wm_resizable(False, False)           # Задать запрет на изменение размера окна
window.iconbitmap("nose_icon.ico")          # Задать иконку окна
window.configure(background='#858F8E')      # Задать фон окна
...
# BUTTON
btn = ttk.Button(window, text="Выбрать", command=getRes, width=40)
